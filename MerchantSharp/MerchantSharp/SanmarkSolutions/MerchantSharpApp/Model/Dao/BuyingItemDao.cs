...
namespace MerchantSharp.SanmarkSolutions.MerchantSharpApp.Model.Dao {
	class BuyingItemDao : BaseDao, IDao {

		private static BuyingItemDao dao = null;

		private BuyingItemDao() {
			base.TableName = "buying_item";
		}

		public static BuyingItemDao getInstance() {
			try {
				if(dao == null) {
					dao = new BuyingItemDao();
					dao.initializeTable();
				}
			} catch(Exception) {
			}
