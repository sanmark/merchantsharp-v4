...
namespace MerchantSharp.SanmarkSolutions.MerchantSharpApp.Model.Dao {
	class SellingOtherDao : BaseDao, IDao {

		private static SellingOtherDao dao = null;

		private SellingOtherDao() {
			base.TableName = "selling_other";
		}

		public static SellingOtherDao getInstance() {
			try {
				if(dao == null) {
					dao = new SellingOtherDao();
					dao.initializeTable();
				}
			} catch(Exception) {
			}
