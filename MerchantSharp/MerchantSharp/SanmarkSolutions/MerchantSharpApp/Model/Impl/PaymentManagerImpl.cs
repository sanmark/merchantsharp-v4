...
namespace MerchantSharp.SanmarkSolutions.MerchantSharpApp.Model.Impl {
	class PaymentManagerImpl {
		...
		internal void printCheque() {
			try {
				String to = "Cash";
				if ( paymentSection.Type == "BuyingInvoice" ) {
					BuyingInvoice bi = buyingInvoiceManagerImpl.getInvoiceById(paymentSection.InvoiceId);
					to = vendorManagerImpl.getVendorNameById(bi.VendorId);
				} else if ( paymentSection.Type == "SellingInvoice" ) {
					SellingInvoice bi = sellingInvoiceManagerImpl.getInvoiceById(paymentSection.InvoiceId);
					to = customerManagerImpl.getCustomerNameById(bi.CustomerId);
				}
				if ( paymentSection.datePicker_payableDate_chequePayments.SelectedDate != null && paymentSection.textBox_amount_chequePayments.DoubleValue > 0 ) {					
					ThreadPool.openTab(new ChequePrint(Convert.ToDateTime(paymentSection.datePicker_payableDate_chequePayments.SelectedDate).ToString("yyyy-MM-dd"), to, paymentSection.textBox_amount_chequePayments.DoubleValue.ToString("0.00")), "Cheque Print");
				} else if ( paymentSection.dataGrid_chequePayments_chequePayments.SelectedItemID > 0 ) {
					DataRow dr = paymentSection.DataTableChequePayments.Rows[paymentSection.dataGrid_chequePayments_chequePayments.SelectedIndex];
					ThreadPool.openTab(new ChequePrint(dr["Payble Date"].ToString(), to, Convert.ToDouble(dr["Amount"]).ToString("0.00")), "Cheque Print");
				}
			} catch ( Exception ) {
			}
		}
